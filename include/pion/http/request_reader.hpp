...
public:
    ...
    // default destructor
    virtual ~request_reader() = default;
...
protected:
    ...
    virtual void read_bytes() {
        get_connection()->async_read_some(std::bind(static_cast<void (request_reader::*)(const std::error_code&, std::size_t)>(&request_reader::consume_bytes),
                                                        shared_from_this(),
                                                        std::placeholders::_1,
                                                        std::placeholders::_2));
    }
