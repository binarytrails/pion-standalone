...
#include <pion/utils/pion_thread.hpp>
...
public:
    ...
    inline void set_num_threads(const pion::uint32_t n) { m_num_threads = n; }
    
    /// returns the number of threads currently in use
    inline pion::uint32_t get_num_threads(void) const { return m_num_threads; }
    ...
    virtual pion::asio::io_service& get_io_service(void) = 0;
     ...
     */
    virtual void post(pion::function<void ()> work_func) {
     ...
     * @param my_timer timer used to keep the IO service active while running
     */
    void keep_running(pion::asio::io_service& my_service,
                     pion::asio::system_timer& my_timer);
    ...
    inline static void sleep(uint32_t sleep_sec, uint32_t sleep_nsec) {
        pion::chrono::system_clock::time_point wakeup_time(get_wakeup_time(sleep_sec, sleep_nsec));
        pion::this_thread::sleep_until(wakeup_time);
    }
    ...
    inline static void sleep(ConditionType& wakeup_condition, LockType& wakeup_lock,
                             pion::uint32_t sleep_sec, pion::uint32_t sleep_nsec)
    {
        pion::chrono::system_clock::time_point wakeup_time(get_wakeup_time(sleep_sec, sleep_nsec));
    ...
    void process_service_work(pion::asio::io_service& service);


protected:

    /**
     * calculates a wakeup time in chrono::system_clock format
     *
     * @param sleep_sec number of seconds to sleep for
     * @param sleep_nsec number of nanoseconds to sleep for
     *
     * @return pion::chrono::system_clock::time_point time to wake up from sleep
     */
    static pion::chrono::system_clock::time_point get_wakeup_time(uint32_t sleep_sec,
        uint32_t sleep_nsec);
    ...
    static const pion::uint32_t    DEFAULT_NUM_THREADS;

    /// number of nanoseconds in one full second (10 ^ 9)
    static const pion::uint32_t    NSEC_IN_SECOND;

    /// number of microseconds in one full second (10 ^ 6)
    static const pion::uint32_t    MICROSEC_IN_SECOND;
    
    /// number of seconds a timer should wait for to keep the IO services running
    static const pion::uint32_t    KEEP_RUNNING_TIMER_SECONDS;
...
protected:
    ...
    typedef std::vector<pion::shared_ptr<pion::thread> >  ThreadPool;
...
public:
    ...
    /// returns an async I/O service used to schedule work
    virtual pion::asio::io_service& get_io_service(void) { return m_service; }
...
protected:
    ...
    pion::asio::io_service         m_service;
...
public:
    ...
    /// returns an async I/O service used to schedule work
    virtual pion::asio::io_service& get_io_service(void) {
        pion::unique_lock<pion::mutex> scheduler_lock(m_mutex);
        while (m_service_pool.size() < m_num_threads) {
            pion::shared_ptr<service_pair_type>  service_ptr(new service_pair_type());
     ...
     */
    virtual pion::asio::io_service& get_io_service(pion::uint32_t n) {
...
protected:
    ...
    /// typedef for a pair object where first is an IO service and second is a timer
    struct service_pair_type {
        service_pair_type(void) : first(), second(first) {}
        pion::asio::io_service         first;
        pion::asio::system_timer     second;
    };
    
    /// typedef for a pool of IO services
    typedef std::vector<pion::shared_ptr<service_pair_type> >        service_pool_type;
