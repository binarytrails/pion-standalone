...
protected:
     ...
     *
     * @return std::chrono::system_clock::time_point time to wake up from sleep
     */
...
public:
    ...
    multi_thread_scheduler() = default;

    /// virtual destructor
    virtual ~multi_thread_scheduler() = default;
...
protected:
    ...
    /// finishes all threads used to perform work
    virtual void finish_threads() { m_thread_pool.clear(); }
...
public:
    ...
    /// returns an async I/O service used to schedule work
    virtual asio::io_service& get_io_service() { return m_service; }
...
protected:
    ...
    /// stops all services used to schedule work
    virtual void stop_services() { m_service.stop(); }

    /// finishes all services used to schedule work
    virtual void finish_services() { m_service.reset(); }
