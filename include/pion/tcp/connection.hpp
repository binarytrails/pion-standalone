...
public:
    ...
    public:
        ...
        inline socket_type& next_layer() { return m_socket; }
        inline const socket_type& next_layer() const { return m_socket; }
        inline socket_type::lowest_layer_type& lowest_layer() { return m_socket.lowest_layer(); }
        inline const socket_type::lowest_layer_type& lowest_layer() const { return m_socket.lowest_layer(); }
     ...
     * @see asio::basic_socket_acceptor::connect()
     */
    inline std::error_code connect(const std::string& remote_server,
                                             const unsigned int remote_port)
    {
     ...
     */
    inline std::error_code handshake_client() {
        std::error_code ec;
#ifdef PION_HAVE_SSL
     ...
     * @see asio::ssl::stream::handshake()
     */
    inline std::error_code handshake_server() {
        std::error_code ec;
#ifdef PION_HAVE_SSL
     ...
     */
    inline std::size_t read_some(std::error_code& ec) {
#ifdef PION_HAVE_SSL
        if (get_ssl_flag())
    ...
    /// the connection
    inline void finish() { if (m_finished_handler) m_finished_handler(shared_from_this()); }
    ...
    inline lifecycle_type get_lifecycle() const { return m_lifecycle; }

    /// returns true if the connection should be kept alive
    inline bool get_keep_alive() const { return m_lifecycle != LIFECYCLE_CLOSE; }

    /// returns true if the HTTP requests are pipelined
    inline bool get_pipelined() const { return m_lifecycle == LIFECYCLE_PIPELINED; }

    /// returns the buffer used for reading data from the TCP connection
    inline read_buffer_type& get_read_buffer() { return m_read_buffer; }
    ...
    inline asio::ip::tcp::endpoint get_remote_endpoint() const {
    ...
    inline unsigned short get_remote_port() const {
    ...
    inline socket_type& get_socket() { return m_ssl_socket.next_layer(); }

    /// returns non-const reference to underlying SSL socket object
    inline ssl_socket_type& get_ssl_socket() { return m_ssl_socket; }

    /// returns const reference to underlying TCP socket object
    inline const socket_type& get_socket() const { return const_cast<ssl_socket_type&>(m_ssl_socket).next_layer(); }

    /// returns const reference to underlying SSL socket object
    inline const ssl_socket_type& get_ssl_socket() const { return m_ssl_socket; }
