...
static char const* rst_stream_status(pion::uint32_t rst_stream_status_code)
{
...
bool parser::is_spdy_control_frame(const char *ptr)
{
    ...
    // Get the control bit
    pion::uint8_t control_bit;
    pion::uint16_t version, type;
    pion::uint16_t byte_value = algorithm::to_uint16(ptr);
    ...
    pion::uint16_t two_bytes = algorithm::to_uint16(ptr);
...
spdy_frame_type parser::get_spdy_frame_type(const char *ptr)
{
    ...
    pion::uint8_t first_byte = *((unsigned char *)ptr);
    if(first_byte == 0x80){
...
pion::uint32_t parser::get_control_frame_stream_id(const char *ptr)
{
    // The stream ID for control frames is at a 8 bit offser from start
    ptr += 8;

    pion::uint32_t four_bytes = algorithm::to_uint32(ptr);
    return four_bytes & 0x7FFFFFFF;
}
    
pion::tribool parser::parse_spdy_frame(pion::error_code& ec,
                                        const decompressor_ptr& decompressor,
                                        ...
                                        pion::uint32_t current_stream_count)
{
    ...
    pion::uint8_t first_byte = (pion::uint8_t)*m_read_ptr;
    ...
    control_bit = (pion::uint8_t)frame.control_bit;
...
bool parser::populate_frame(pion::error_code& ec,
                            spdy_control_frame_info& frame,
                            pion::uint32_t& length_packet,
                            ...
                            http_protocol_info& http_info)
{
    // Get the control bit
    pion::uint8_t control_bit;
    pion::uint16_t byte_value = algorithm::to_uint16(m_read_ptr);
    control_bit = byte_value >> (sizeof(short) * CHAR_BIT - 1);
    ...
    if(control_bit){
        ...
        // Get the version number
        pion::uint16_t two_bytes = algorithm::to_uint16(m_read_ptr);
        ...
        // Get the stream id
        pion::uint32_t four_bytes = algorithm::to_uint32(m_read_ptr);
    ...
    // Get the flags
    frame.flags = (pion::uint8_t)*m_read_ptr;
...
void parser::parse_header_payload(pion::error_code &ec,
                                  const decompressor_ptr& decompressor,
...
{
    pion::uint32_t stream_id = 0;
    pion::uint32_t header_block_length = frame.length;
    ...
    pion::uint16_t num_name_val_pairs = algorithm::to_uint16(m_uncompressed_ptr);
    ...
    for(pion::uint16_t count = 0; count < num_name_val_pairs; ++count){
        
        
        // Get the length of the name
        pion::uint16_t length_name = algorithm::to_uint16(m_uncompressed_ptr);
        std::string name = "";
        
        m_uncompressed_ptr += 2;
        
        {
            for(pion::uint16_t count = 0; count < length_name; ++count){
        ...
        pion::uint16_t length_value = algorithm::to_uint16(m_uncompressed_ptr);
        std::string value = "";
        
        m_uncompressed_ptr += 2;
        
        {
            for(pion::uint16_t count = 0; count < length_value; ++count){
...
void parser::parse_spdy_data(pion::error_code& /* ec */,
                             const spdy_control_frame_info& frame,
                             pion::uint32_t /* stream_id */,
                             http_protocol_info& http_info)
{
...
void parser::parse_spdy_rst_stream(pion::error_code& /* ec */,
                                   const spdy_control_frame_info& frame)
{
    pion::uint32_t status_code = 0;
...
void parser::parse_spdy_ping_frame(pion::error_code& /* ec */,
                                   const spdy_control_frame_info& frame)
{
...
void parser::parse_spdy_settings_frame(pion::error_code& /* ec */,
                                       const spdy_control_frame_info& /* frame */)
{
    // Can ignore this frame for our purposes
}

void parser::parse_spdy_goaway_frame(pion::error_code &ec,
                                     const spdy_control_frame_info& frame)
{
    // First complete the check for size
    // The length should be 4 always
...
void parser::parse_spdy_window_update_frame(pion::error_code& /* ec */,
                                            const spdy_control_frame_info& /* frame */)
{
