...
const pion::uint32_t   scheduler::DEFAULT_NUM_THREADS = 8;
const pion::uint32_t   scheduler::NSEC_IN_SECOND = 1000000000; // (10^9)
const pion::uint32_t   scheduler::MICROSEC_IN_SECOND = 1000000;    // (10^6)
const pion::uint32_t   scheduler::KEEP_RUNNING_TIMER_SECONDS = 5;
...
void scheduler::keep_running(pion::asio::io_service& my_service,
                                pion::asio::system_timer& my_timer)
{
    if (m_is_running) {
        // schedule this again to make sure the service doesn't complete
        my_timer.expires_from_now(pion::chrono::seconds(KEEP_RUNNING_TIMER_SECONDS));
        my_timer.async_wait(pion::bind(&scheduler::keep_running, this,
                                        pion::ref(my_service), pion::ref(my_timer)));
    }
}
...
pion::chrono::system_clock::time_point scheduler::get_wakeup_time(uint32_t sleep_sec,
    uint32_t sleep_nsec)
{
    return pion::chrono::system_clock::now() + pion::chrono::seconds(sleep_sec) + pion::chrono::microseconds(sleep_nsec / 1000);
}
                     
void scheduler::process_service_work(pion::asio::io_service& service) {
    while (m_is_running) {
        try {
            service.run();
        } catch (std::exception& e) {
            PION_LOG_ERROR(m_logger, pion::diagnostic_information(e));
        } catch (...) {
...
void single_service_scheduler::startup(void)
{
    ...
    if (! m_is_running) {
        ...
        for (pion::uint32_t n = 0; n < m_num_threads; ++n) {
            pion::shared_ptr<pion::thread> new_thread(new pion::thread( pion::bind(&scheduler::process_service_work,
                                                                                       this, pion::ref(m_service)) ));
...
void one_to_one_scheduler::startup(void)
{
    ...
    if (! m_is_running) {
        ...
        for (pion::uint32_t n = 0; n < m_num_threads; ++n) {
            pion::shared_ptr<pion::thread> new_thread(new pion::thread( pion::bind(&scheduler::process_service_work,
                                                                                       this, pion::ref(m_service_pool[n]->first)) ));
            m_thread_pool.push_back(new_thread);
        }
    }
}
